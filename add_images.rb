...
laptop_image_urls = [
  ...
  'https://images.unsplash.com/photo-1502740479091-635887520276?w=500',
  'https://images.unsplash.com/photo-1519389950473-47ba0277781c?w=500'
]

all_products = Product.all
puts "Total products: #{all_products.count}"

all_products.each_with_index do |product, index|
  begin
    if product.image.attached?
      puts "Skipping #{product.name} - already has image"
    else
...
puts "Done! #{Product.all.select { |p| p.image.attached? }.count} products now have images"
